...
class TdlistsContainer extends Component {
	constructor(props) {
	  ...
	  loadTdlists() {
		axios.get('http://localhost:3000/api/v1/tdslist')
		.then(res => {
	  ...
	  newTdlist = (e) => {
		if (e.key === 'Enter' && !(e.target.value === '')) {
		  axios.post('http://localhost:3000/api/v1/tdslist', {tdlist: {title: e.target.value}})
		  .then(res => {
	  ...
	  modifyTdlist = (e, id) => {
		axios.put(`http://localhost:3000/api/v1/tdslist/${id}`, {tdlist: {done: e.target.checked}})
		.then(res => {
	  ...
	  removeTdlist = (id) => {
		axios.delete(`http://localhost:3000/api/v1/tdslist/${id}`)
		.then(res => {
